...
window.localStorage.setItem('BIOS_VER', BIOS_VERSION)
...
const renderBootMenu = () => {
  ...
  for (const [bootLoader, directory] of Object.entries(bootLoaderDirectories)) {
    const bootLoaderButton = document.createElement('button')
    bootLoaderButton.textContent = bootLoader
    bootLoaderButton.style.display = 'block'
    bootLoaderButton.style.margin = '10px'
    bootLoaderButton.style.padding = '10px'
    bootLoaderButton.style.border = 'none'
    bootLoaderButton.style.backgroundColor = 'white'
    bootLoaderButton.style.color = 'black'
...
const timeout = setTimeout(() => {
  // @ts-expect-error
  window.loadBootLoader(Object.values(bootLoaderDirectories)[0]).catch(console.error)
  unloadBIOS()
}, 2500)
